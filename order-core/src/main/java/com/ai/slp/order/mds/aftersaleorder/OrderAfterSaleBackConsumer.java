...
public class OrderAfterSaleBackConsumer extends AbstractMdsConsumer {
	...
	public void startMdsConsumer() throws Exception {
		logger.error("开始启动OrderAfterSaleBackMdsConsumer。。。。。");
        ...
        IMessageConsumer msgConsumer= MDSClientFactory.getConsumerClient(
                OrdersConstants.MDSNS.MDS_NS_AFTERSALEORDER_BACK_TOPIC, msgProcessorHandler);
        msgConsumer.start();
        logger.error("成功启动OrderAfterSaleBackMdsConsumer。。。。。");
