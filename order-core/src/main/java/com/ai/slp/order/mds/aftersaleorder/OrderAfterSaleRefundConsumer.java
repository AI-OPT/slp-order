...
//@Component
public class OrderAfterSaleRefundConsumer extends AbstractMdsConsumer {
	private static Logger logger = LoggerFactory.getLogger(OrderAfterSaleRefundConsumer.class);

	@Autowired
	private IOrderAfterSaleBusiSV orderAfterSaleBusiSV;
	@Override
	public void startMdsConsumer() throws Exception {
		logger.error("开始启动OrderAfterSaleRefundMdsConsumer。。。。。");
		IMsgProcessorHandler msgProcessorHandler=new IMsgProcessorHandler() {
            ...
            public IMessageProcessor[] createInstances(int paramInt) {
                ...
                for (int i = 0; i < paramInt; i++) {
                    processor = new OrderAfterSaleRefundMessProcessorImpl(orderAfterSaleBusiSV);
        ...
        IMessageConsumer msgConsumer= MDSClientFactory.getConsumerClient(
                OrdersConstants.MDSNS.MDS_NS_AFTERSALEORDER_REFUND_TOPIC, msgProcessorHandler);
        msgConsumer.start();
        logger.error("成功启动OrderAfterSaleRefundMdsConsumer。。。。。");
