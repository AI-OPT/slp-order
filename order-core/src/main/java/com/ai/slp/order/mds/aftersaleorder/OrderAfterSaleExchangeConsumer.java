...
//@Component
public class OrderAfterSaleExchangeConsumer extends AbstractMdsConsumer {
	private static Logger logger = LoggerFactory.getLogger(OrderAfterSaleExchangeConsumer.class);

	@Autowired
	private IOrderAfterSaleBusiSV orderAfterSaleBusiSV;
	@Override
	public void startMdsConsumer() throws Exception {
		logger.error("开始启动OrderAfterSaleExchangeMdsConsumer。。。。。");
		IMsgProcessorHandler msgProcessorHandler=new IMsgProcessorHandler() {
            ...
            public IMessageProcessor[] createInstances(int paramInt) {
                ...
                for (int i = 0; i < paramInt; i++) {
                    processor = new OrderAfterSaleExchangeMessProcessorImpl(orderAfterSaleBusiSV);
        ...
        IMessageConsumer msgConsumer= MDSClientFactory.getConsumerClient(
                OrdersConstants.MDSNS.MDS_NS_AFTERSALEORDER_EXCHANGE_TOPIC, msgProcessorHandler);
        msgConsumer.start();
        logger.error("成功启动OrderAfterSaleExchangeMdsConsumer。。。。。");
