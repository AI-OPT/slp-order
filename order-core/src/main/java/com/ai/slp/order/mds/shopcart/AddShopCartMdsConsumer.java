...
@Component
public class AddShopCartMdsConsumer extends AbstractMdsConsumer {
	private static Logger logger = LoggerFactory.getLogger(AddShopCartMdsConsumer.class);

	@Autowired
	IShopCartBusiSV shopCartBusiSV;
	@Override
	public void startMdsConsumer() throws Exception {
		logger.error("开始启动AddShopCartMdsConsumer。。。。。");
		IMsgProcessorHandler msgProcessorHandler=new IMsgProcessorHandler() {
            ...
            public IMessageProcessor[] createInstances(int paramInt) {
                ...
                for (int i = 0; i < paramInt; i++) {
                    processor = new AddShopCartMessProcessorImpl(shopCartBusiSV);
        ...
        IMessageConsumer msgConsumer= MDSClientFactory.getConsumerClient(
                OrdersConstants.MDSNS.MDS_NS_SHOPCART_ADD_TOPIC, msgProcessorHandler);
        msgConsumer.start();
        logger.error("成功启动AddShopCartMdsConsumer。。。。。");
